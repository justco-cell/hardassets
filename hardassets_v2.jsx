...
const gBtnRef=useRef(null);const[gisLoaded,setGisLoaded]=useState(false);const[gisFailed,setGisFailed]=useState(false);
const onLoginRef=useRef(onLogin);
onLoginRef.current=onLogin;
const[gE,setGE]=useState("");const[gN,setGN]=useState("");const[gMode,setGMode]=useState(false);
useEffect(()=>{
...
const timer=setTimeout(()=>setGisFailed(true),3000);
return()=>{clearTimeout(timer);try{document.head.removeChild(script)}catch(e){}};
},[]);
useEffect(()=>{
if(!gisLoaded||!window.google||!gBtnRef.current)return;
try{
...
}catch(e){onLoginRef.current({email:"google-user",name:"Google User",method:"google"})}
}
});
...
{gisLoaded&&!gisFailed?<div ref={gBtnRef}/>:gisFailed?(!gMode?<button onClick={()=>setGMode(true)} style={{width:"100%",padding:"11px 0",borderRadius:8,border:"1px solid "+T.bdr,background:"#fff",color:"#3c4043",fontSize:13,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:10}}><GIc/> Sign in with Google</button>:<div style={{width:"100%",padding:12,border:"1px solid "+T.bdr,borderRadius:10,background:T.bgI}}><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}><GIc/><span style={{fontSize:13,fontWeight:600,color:T.txt}}>Google Sign In</span></div><div style={{display:"flex",flexDirection:"column",gap:8}}><In value={gN} onChange={setGN} placeholder="Name"/><In value={gE} onChange={setGE} placeholder="your.email@gmail.com"/><div style={{display:"flex",gap:6}}><Bt onClick={()=>{if(gE)onLogin({email:gE,name:gN||gE.split("@")[0],method:"google"})}} style={{flex:1,padding:"9px 0"}}>Continue</Bt><Bt ghost onClick={()=>setGMode(false)} style={{padding:"9px 12px"}}>Back</Bt></div></div></div>):<div style={{width:"100%",padding:"11px 0",borderRadius:8,border:"1px solid "+T.bdr,background:"#fff",color:"#3c4043",fontSize:13,fontWeight:600,textAlign:"center",display:"flex",alignItems:"center",justifyContent:"center",gap:10}}><GIc/> Loading Google...</div>}
</div>
