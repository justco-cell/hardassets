...
export default function HardAssetsWeb(){
  ...
  return<div style={{fontFamily:ff,background:P.bg,color:P.text,minHeight:"100vh",display:"flex"}}>
    ...
    {/* MODALS */}
    <Modal open={modal==="addMetal"} onClose={()=>setModal(null)} title="Add Metal Holding"><AddMetalForm prices={prices} onAdd={m=>{setMetals(p=>[...p,m]);logActivity(authToken,"add","metal",m.id,m.name||m.metal,null,m)}} onClose={()=>setModal(null)}/></Modal>
    <Modal open={modal==="addSynd"} onClose={()=>setModal(null)} title="Add Syndication"><AddSyndForm synds={synds} onAdd={s=>{setSynds(p=>[...p,s]);logActivity(authToken,"add","synd",s.id,s.name,null,s)}} onClose={()=>setModal(null)}/></Modal>
    <Modal open={modal==="addCrypto"} onClose={()=>setModal(null)} title="Add Crypto"><AddCryptoForm prices={prices} allCrypto={allCrypto} onAdd={c=>{setCrypto(p=>[...p,c]);logActivity(authToken,"add","crypto",c.id,c.name||c.coin,null,c)}} onClose={()=>setModal(null)}/></Modal>
    <Modal open={modal==="addProperty"} onClose={()=>setModal(null)} title="Add Property"><AddPropertyForm onAdd={p=>{setProperties(prev=>[...prev,p]);logActivity(authToken,"add","property",p.id,p.name,null,p)}} onClose={()=>setModal(null)}/></Modal>
    <Modal open={modal==="addNote"} onClose={()=>setModal(null)} title="Add Note"><AddNoteForm onAdd={n=>{setNotesLending(prev=>[...prev,n]);logActivity(authToken,"add","note",n.id,n.name,null,n)}} onClose={()=>setModal(null)}/></Modal>
    <Modal open={modal==="addCollectible"} onClose={()=>setModal(null)} title="Add Collectible"><AddCollectibleForm onAdd={c=>{setCollectibles(prev=>[...prev,c]);logActivity(authToken,"add","collectible",c.id,c.name,null,c)}} onClose={()=>setModal(null)}/></Modal>
    ...
    <Modal open={!!confirmDelete} onClose={()=>setConfirmDelete(null)} title="Confirm Delete">
      <div style={{textAlign:"center",padding:"12px 0"}}>
        ...
        <div style={{display:"flex",gap:12,justifyContent:"center"}}>
          ...
          <Btn variant="danger" onClick={()=>{deleteItem(confirmDelete.id,confirmDelete.type);logActivity(authToken,"delete",confirmDelete.type,confirmDelete.id,confirmDelete.name,confirmDelete,null);setConfirmDelete(null);setEditItem(null)}}>Delete</Btn>
        </div>
      </div>
    </Modal>
