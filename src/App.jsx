...
export default function HardAssetsWeb(){
  ...
  const TH=({cols})=><div style={{display:"grid",gridTemplateColumns:cols.map(c=>c.w||"1fr").join(" "),gap:"0 12px",padding:"10px 20px",borderBottom:`1px solid ${P.border}`}}>{cols.map((c,i)=><span key={i} style={{fontSize:11,fontWeight:600,color:P.txM,letterSpacing:0.3,textTransform:"uppercase",textAlign:c.right?"right":"left"}}>{c.label}</span>)}</div>;
