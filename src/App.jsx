...
function ContactPg({onNav}){
  ...
  const submit=async()=>{if(!f.name||!f.email||!f.msg)return;setSending(true);try{await fetch("https://formsubmit.co/ajax/support@hardassets.io",{method:"POST",headers:{"Content-Type":"application/json","Accept":"application/json"},body:JSON.stringify({name:f.name,email:f.email,message:f.msg,_subject:"HardAssets.io Contact: "+f.name,_template:"table"})})}catch(e){}try{await fetch("/api/contact",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:f.name,email:f.email,message:f.msg})})}catch(e){}setSent(true);setSending(false)};
