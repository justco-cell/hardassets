...
async function fetchMetalPrices(){try{const r=await fetch("/api/prices");if(!r.ok)return null;const d=await r.json();if(d.status!=="success"||!d.metals)return null;return{gold:d.metals.gold||0,silver:d.metals.silver||0,platinum:d.metals.platinum||0,palladium:d.metals.palladium||0,goldChg:d.changes?.gold||0,silverChg:d.changes?.silver||0,platChg:d.changes?.platinum||0,palladiumChg:d.changes?.palladium||0}}catch(e){return null}}
...
function DealAnalyzer(){const[d,sD]=useState({price:"",rent:"",tax:"",insurance:"",maintenance:"",vacancy:"8",mgmt:"10",downPct:"25",rate:"7",term:"30",holdYrs:"5",closingPct:"3",appreciation:"3"});
  ...
  // Equity Multiple: total return over hold period / total cash invested
  const holdYrs=+d.holdYrs||5,appRate=+d.appreciation||0;
  const futureValue=price*Math.pow(1+appRate/100,holdYrs);
  const totalCF=cf*holdYrs;
  // Remaining loan balance after holdYrs
  ...
  const saleProceeds=futureValue-loanBalance-(futureValue*0.06); // 6% selling costs
  const totalReturn=totalCF+saleProceeds;
  const equityMultiple=totalCashIn>0?totalReturn/totalCashIn:0;
  // Break-Even Occupancy
  const totalExpenses=tax+ins+(maint*12)+mgmtC;
  const breakEvenOcc=gross>0?((totalExpenses+(mortgage*12))/gross)*100:0;
  const tests=[{name:"Cap Rate",val:cap.toFixed(1)+"%",pass:cap>=6,tgt:"≥ 6%"},{name:"CoC Return",val:coc.toFixed(1)+"%",pass:coc>=8,tgt:"≥ 8%"},{name:"DSCR",val:dscr.toFixed(2),pass:dscr>=1.25,tgt:"≥ 1.25"},{name:"Monthly CF",val:fmt(mcf),pass:mcf>0,tgt:"> $0"},{name:"Equity Multiple",val:equityMultiple.toFixed(2)+"x",pass:equityMultiple>=1.8,tgt:"≥ 1.8x"},{name:"Break-Even Occ.",val:breakEvenOcc.toFixed(0)+"%",pass:breakEvenOcc<=85,tgt:"≤ 85%"}];
  return<div style={{display:"grid",gridTemplateColumns:price>0?"1fr 1fr":"1fr",gap:20}}>
    <GC style={{padding:24}}><div style={{fontSize:16,fontWeight:700,color:P.text,marginBottom:18}}>Property Details</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><FF label="Purchase Price" value={d.price} onChange={v=>sD({...d,price:v})} prefix="$" type="number" isMono/><FF label="Monthly Rent" value={d.rent} onChange={v=>sD({...d,rent:v})} prefix="$" type="number" isMono/><FF label="Tax/yr" value={d.tax} onChange={v=>sD({...d,tax:v})} prefix="$" type="number" isMono/><FF label="Insurance/yr" value={d.insurance} onChange={v=>sD({...d,insurance:v})} prefix="$" type="number" isMono/><FF label="Maintenance/mo" value={d.maintenance} onChange={v=>sD({...d,maintenance:v})} prefix="$" type="number" isMono/><FF label="Vacancy %" value={d.vacancy} onChange={v=>sD({...d,vacancy:v})} prefix="%" type="number" isMono/><FF label="Mgmt %" value={d.mgmt} onChange={v=>sD({...d,mgmt:v})} prefix="%" type="number" isMono/><FF label="Down Payment %" value={d.downPct} onChange={v=>sD({...d,downPct:v})} prefix="%" type="number" isMono/><FF label="Loan Rate %" value={d.rate} onChange={v=>sD({...d,rate:v})} prefix="%" type="number" isMono/><FF label="Loan Term (yrs)" value={d.term} onChange={v=>sD({...d,term:v})} type="number" isMono/><FF label="Closing Costs %" value={d.closingPct} onChange={v=>sD({...d,closingPct:v})} prefix="%" type="number" isMono/><FF label="Hold Period (yrs)" value={d.holdYrs} onChange={v=>sD({...d,holdYrs:v})} type="number" isMono/><FF label="Appreciation %/yr" value={d.appreciation} onChange={v=>sD({...d,appreciation:v})} prefix="%" type="number" isMono/></div></GC>
    {price>0&&<div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:18}}>{tests.map((t,i)=><GC key={i} style={{padding:"16px 18px",borderColor:t.pass?"rgba(52,211,153,0.12)":"rgba(248,113,113,0.12)"}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}><span style={{fontSize:12,color:P.txS}}>{t.name}</span><span style={{fontSize:10,color:P.txM}}>{t.tgt}</span></div><div style={{fontSize:22,fontWeight:800,color:t.pass?P.green:P.red,fontFamily:mono}}>{t.val}</div><div style={{fontSize:11,fontWeight:700,color:t.pass?P.green:P.red,marginTop:4}}>{t.pass?"✓ Pass":"✗ Fail"}</div></GC>)}</div>
      <GC style={{padding:24}}><div style={{fontSize:14,fontWeight:700,color:P.text,marginBottom:12}}>Cash Flow Breakdown</div>{[["Gross Rent",gross],["Vacancy",-vac],["Management",-mgmtC],["NOI",noi,1],["Mortgage/yr",-(mortgage*12)],["Annual CF",cf,1],["Monthly CF",mcf,1]].map(([l,v,b],i)=><div key={i} style={{display:"flex",justifyContent:"space-between",padding:"9px 0",borderTop:b?`1px solid ${P.border}`:"none"}}><span style={{fontSize:13,color:b?P.text:P.txS,fontWeight:b?600:400}}>{l}</span><span style={{fontSize:14,fontWeight:b?700:500,color:v>=0?(b?P.green:P.text):P.red,fontFamily:mono}}>{fmt(v)}</span></div>)}</GC>
      <GC style={{padding:24,marginTop:12}}><div style={{fontSize:14,fontWeight:700,color:P.text,marginBottom:12}}>Investment Summary ({holdYrs}yr hold)</div>{[["Total Cash In",totalCashIn],["Down Payment",down],["Closing Costs",closingCost],["Total Cash Flow",totalCF,1],["Est. Sale Price",futureValue],["Loan Balance at Sale",-loanBalance],["Sale Proceeds (net 6%)",saleProceeds,1],["Total Return",totalReturn,1],["Equity Multiple",null,1,equityMultiple.toFixed(2)+"x"]].map(([l,v,b,txt],i)=><div key={i} style={{display:"flex",justifyContent:"space-between",padding:"9px 0",borderTop:b?`1px solid ${P.border}`:"none"}}><span style={{fontSize:13,color:b?P.text:P.txS,fontWeight:b?600:400}}>{l}</span><span style={{fontSize:14,fontWeight:b?700:500,color:txt?P.gold:v>=0?(b?P.green:P.text):P.red,fontFamily:mono}}>{txt||fmt(v)}</span></div>)}</GC></div>}
  </div>}
...
export default function HardAssetsWeb(){
  ...
  const refreshPrices=async()=>{setRefreshing(true);const[mp,cp,fx]=await Promise.all([fetchMetalPrices(),fetchCryptoPrices(),fetchForex()]);if(fx)setForex(fx);setPrices(prev=>{const next={...prev};if(mp){next.gold=mp.gold;next.silver=mp.silver;next.platinum=mp.platinum;next.palladium=mp.palladium;next.goldChg=mp.goldChg||0;next.silverChg=mp.silverChg||0;next.platChg=mp.platChg||0}if(cp){if(cp.BTC){next.btc=cp.BTC.price;next.btcChg=cp.BTC.change}if(cp.ETH){next.eth=cp.ETH.price;next.ethChg=cp.ETH.change}if(cp.SOL){next.sol=cp.SOL.price;next.solChg=cp.SOL.change}}return next});if(cp){const ac={};for(const[sym,data] of Object.entries(cp)){ac[sym]=data.price}setAllCrypto(ac)}if(mp)setMetals(prev=>prev.map(m=>{const ls=({Gold:mp.gold,Silver:mp.silver,Platinum:mp.platinum,Palladium:mp.palladium})[m.metal];return ls?{...m,spot:Math.round(ls*100)/100}:m}));if(cp)setCrypto(prev=>prev.map(c=>{const l=cp[c.coin];return l?{...c,price:l.price}:c}));setLastRefresh(new Date().toLocaleTimeString());if(window.posthog)window.posthog.capture('prices_refreshed');setRefreshing(false)};
