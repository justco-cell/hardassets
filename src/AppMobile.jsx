...
export default function HardAssets(){
  ...
  const renderTab=()=>{switch(tab){
    ...
    case "markets": {
      ...
      return <div style={{paddingBottom:120}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:10}}>
          <GC style={{padding:"12px 10px",textAlign:"center"}}><div style={{fontSize:9,color:P.txM,fontWeight:600,letterSpacing:0.4,textTransform:"uppercase",marginBottom:4}}>Silver</div><div style={{fontSize:15,fontWeight:800,color:P.text,fontFamily:mono}}>${prices.silver.toFixed(2)}</div><div style={{fontSize:10,fontWeight:600,color:prices.silverChg>=0?P.green:P.red,fontFamily:mono}}>{fPct(prices.silverChg)}</div></GC>
          <GC style={{padding:"12px 10px",textAlign:"center"}}><div style={{fontSize:9,color:P.txM,fontWeight:600,letterSpacing:0.4,textTransform:"uppercase",marginBottom:4}}>Platinum</div><div style={{fontSize:15,fontWeight:800,color:P.text,fontFamily:mono}}>${prices.platinum.toFixed(2)}</div><div style={{fontSize:10,fontWeight:600,color:prices.platChg>=0?P.green:P.red,fontFamily:mono}}>{fPct(prices.platChg)}</div></GC>
          <GC style={{padding:"12px 10px",textAlign:"center"}}><div style={{fontSize:9,color:P.txM,fontWeight:600,letterSpacing:0.4,textTransform:"uppercase",marginBottom:4}}>Palladium</div><div style={{fontSize:15,fontWeight:800,color:P.text,fontFamily:mono}}>${prices.palladium.toFixed(2)}</div></GC>
        </div>
    ...
    case "properties": return <div style={{paddingBottom:120}}>
      {(()=>{const totalEquity=properties.reduce((s,x)=>s+((+(x.currentValue||0))-(+(x.mortgageBalance||0))),0);const totalVal=properties.reduce((s,x)=>s+(+(x.currentValue||0)),0);const totalCF=properties.reduce((s,x)=>s+((+(x.monthlyRent||0))-(+(x.monthlyExpenses||0))-(+(x.mortgagePayment||0))),0);return<div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:14}}><GC style={{padding:"14px 12px",textAlign:"center",borderColor:"rgba(52,211,153,0.12)"}}><div style={{fontSize:9,color:P.txM,fontWeight:600,letterSpacing:0.4,textTransform:"uppercase",marginBottom:4}}>Total Equity</div><div style={{fontSize:16,fontWeight:800,color:P.green,fontFamily:mono}}>{fmt(totalEquity)}</div></GC><GC style={{padding:"14px 12px",textAlign:"center",borderColor:"rgba(52,211,153,0.12)"}}><div style={{fontSize:9,color:P.txM,fontWeight:600,letterSpacing:0.4,textTransform:"uppercase",marginBottom:4}}>Total Value</div><div style={{fontSize:16,fontWeight:800,color:P.text,fontFamily:mono}}>{fmt(totalVal)}</div></GC><GC style={{padding:"14px 12px",textAlign:"center",borderColor:"rgba(52,211,153,0.12)"}}><div style={{fontSize:9,color:P.txM,fontWeight:600,letterSpacing:0.4,textTransform:"uppercase",marginBottom:4}}>Monthly CF</div><div style={{fontSize:16,fontWeight:800,color:totalCF>=0?P.green:P.red,fontFamily:mono}}>{fmt(totalCF)}</div></GC></div>})()}
      <TabHeader title="Properties" count={properties.length} onAdd={()=>setSheet("addProperty")} addLabel="+ Add" onExport={()=>csvExport(["Name","Address","Type","Purchase Price","Current Value","Mortgage","Equity","Rent/mo","Expenses/mo","Mortgage/mo","CF/mo","Risk","Date Purchased","Hold Period","Yrs Held","Notes"],properties.map(p=>{const eq=(+(p.currentValue||0))-(+(p.mortgageBalance||0));const cf=(+(p.monthlyRent||0))-(+(p.monthlyExpenses||0))-(+(p.mortgagePayment||0));return[p.name,p.address||"",p.propType,p.purchasePrice,p.currentValue,p.mortgageBalance||0,eq,p.monthlyRent||0,p.monthlyExpenses||0,p.mortgagePayment||0,cf,p.risk||"",p.dateInvested||p.datePurchased||"",p.holdPeriod||"",yrsHeld(p.dateInvested||p.datePurchased),p.notes||""]}),"properties.csv")} onImport={rows=>{const ni=rows.map(r=>({id:uid(),name:r.Name||"",address:r.Address||"",propType:r.Type||"SFH",purchasePrice:+(r["Purchase Price"]||0),currentValue:+(r["Current Value"]||0),mortgageBalance:+(r.Mortgage||0),monthlyRent:+(r["Rent/mo"]||0),monthlyExpenses:+(r["Expenses/mo"]||0),mortgagePayment:+(r["Mortgage/mo"]||0),risk:+(r.Risk||5),dateInvested:r["Date Purchased"]||"",holdPeriod:r["Hold Period"]||"",notes:r.Notes||""})).filter(i=>i.name);setProperties(p=>[...p,...ni]);logActivity(authToken,"import","property",null,"CSV Import",null,{count:ni.length})}}/>
    ...
    case "notes": return <div style={{paddingBottom:120}}>
      {(()=>{const totalDeployed=notesLending.reduce((s,x)=>s+(+(x.principal||0)),0);const monthlyInc=notesLending.reduce((s,x)=>s+((+(x.outstandingBalance||0))*(+(x.interestRate||0))/100/12),0);const avgRate=notesLending.length>0?notesLending.reduce((s,x)=>s+(+(x.interestRate||0)),0)/notesLending.length:0;return<div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:14}}><GC style={{padding:"14px 12px",textAlign:"center",borderColor:"rgba(34,211,238,0.12)"}}><div style={{fontSize:9,color:P.txM,fontWeight:600,letterSpacing:0.4,textTransform:"uppercase",marginBottom:4}}>Deployed</div><div style={{fontSize:16,fontWeight:800,color:P.cyan,fontFamily:mono}}>{fmt(totalDeployed)}</div></GC><GC style={{padding:"14px 12px",textAlign:"center",borderColor:"rgba(34,211,238,0.12)"}}><div style={{fontSize:9,color:P.txM,fontWeight:600,letterSpacing:0.4,textTransform:"uppercase",marginBottom:4}}>Monthly Inc</div><div style={{fontSize:16,fontWeight:800,color:P.green,fontFamily:mono}}>{fmt(monthlyInc)}</div></GC><GC style={{padding:"14px 12px",textAlign:"center",borderColor:"rgba(34,211,238,0.12)"}}><div style={{fontSize:9,color:P.txM,fontWeight:600,letterSpacing:0.4,textTransform:"uppercase",marginBottom:4}}>Avg Rate</div><div style={{fontSize:16,fontWeight:800,color:P.cyan,fontFamily:mono}}>{avgRate.toFixed(1)}%</div></GC></div>})()}
      <TabHeader title="Notes & Lending" count={notesLending.length} onAdd={()=>setSheet("addNote")} addLabel="+ Add" onExport={()=>csvExport(["Name","Type","Principal","Outstanding","Rate%","Maturity","Collateral","Status","LTV","Risk","Date Invested","Hold Period","Yrs Held","Notes"],notesLending.map(n=>[n.name,n.noteType,n.principal,n.outstandingBalance,n.interestRate||"",n.maturityDate||"",n.collateral||"",n.status,n.ltv||"",n.risk||"",n.dateInvested||"",n.holdPeriod||"",yrsHeld(n.dateInvested),n.notes||""]),"notes.csv")} onImport={rows=>{const ni=rows.map(r=>({id:uid(),name:r.Name||"",noteType:r.Type||"Hard Money",principal:+(r.Principal||0),outstandingBalance:+(r.Outstanding||0),interestRate:+(r["Rate%"]||0),maturityDate:r.Maturity||"",collateral:r.Collateral||"",status:r.Status||"Performing",ltv:+(r.LTV||0),risk:+(r.Risk||5),dateInvested:r["Date Invested"]||"",holdPeriod:r["Hold Period"]||"",notes:r.Notes||""})).filter(i=>i.name&&i.principal>0);setNotesLending(p=>[...p,...ni]);logActivity(authToken,"import","note",null,"CSV Import",null,{count:ni.length})}}/>
  ...
  return<div style={{fontFamily:ff,background:P.bg,color:P.text,minHeight:"100vh",maxWidth:430,margin:"0 auto",position:"relative",overflow:"hidden"}}>
    ...
    {/* Sheets */}
    <Sheet open={sheet==="addMetal"} onClose={()=>setSheet(null)} title="Add Metal"><AddMetalForm prices={prices} onAdd={m=>{setMetals(p=>[...p,m]);logActivity(authToken,"add","metal",m.id,m.name||m.metal,null,m)}} onClose={()=>setSheet(null)}/></Sheet>
    <Sheet open={sheet==="addSynd"} onClose={()=>setSheet(null)} title="Add Syndication"><AddSyndForm synds={synds} onAdd={s=>{setSynds(p=>[...p,s]);logActivity(authToken,"add","synd",s.id,s.name||s.sponsor,null,s)}} onClose={()=>setSheet(null)}/></Sheet>
    <Sheet open={sheet==="addCrypto"} onClose={()=>setSheet(null)} title="Add Crypto"><AddCryptoForm prices={prices} allCrypto={allCrypto} onAdd={c=>{setCrypto(p=>[...p,c]);logActivity(authToken,"add","crypto",c.id,c.name||c.coin,null,c)}} onClose={()=>setSheet(null)}/></Sheet>
    <Sheet open={sheet==="addProperty"} onClose={()=>setSheet(null)} title="Add Property"><AddPropertyForm onAdd={p=>{setProperties(prev=>[...prev,p]);logActivity(authToken,"add","property",p.id,p.name,null,p)}} onClose={()=>setSheet(null)}/></Sheet>
    <Sheet open={sheet==="addNote"} onClose={()=>setSheet(null)} title="Add Note"><AddNoteForm onAdd={n=>{setNotesLending(prev=>[...prev,n]);logActivity(authToken,"add","note",n.id,n.name,null,n)}} onClose={()=>setSheet(null)}/></Sheet>
    <Sheet open={sheet==="addCollectible"} onClose={()=>setSheet(null)} title="Add Collectible"><AddCollectibleForm onAdd={c=>{setCollectibles(prev=>[...prev,c]);logActivity(authToken,"add","collectible",c.id,c.name,null,c)}} onClose={()=>setSheet(null)}/></Sheet>
    {/* Edit Sheet */}
    ...
    <Sheet open={!!confirmDelete} onClose={()=>setConfirmDelete(null)} title="Confirm Delete">
      <div style={{textAlign:"center",padding:"12px 0"}}>
        ...
        <div style={{display:"flex",gap:12,justifyContent:"center"}}>
          ...
          <Btn variant="danger" onClick={()=>{logActivity(authToken,"delete",confirmDelete.type,confirmDelete.id,confirmDelete.name,confirmDelete,null);deleteItem(confirmDelete.id,confirmDelete.type);setConfirmDelete(null);setEditItem(null)}}>Delete</Btn>
        </div>
      </div>
    </Sheet>
          ...
          return<div key={i} style={{display:"flex",gap:10,padding:"10px 0",borderBottom:`1px solid ${P.border}`}}>
            ...
            <div style={{flex:1}}>
              <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:2}}>
                <span style={{fontSize:9,padding:"1px 6px",borderRadius:4,background:P.elevated,color:P.txS,fontWeight:600}}>{l.asset_type}</span>
